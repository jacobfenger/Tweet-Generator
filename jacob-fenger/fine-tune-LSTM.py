...
def InOut(unique_chars, chars, token_dict_char_int):
    ...
    x = np.zeros((len(sentences), maxlen, 150), dtype=np.bool)
    y = np.zeros((len(sentences), 150), dtype=np.bool)
...
def getModel(maxlen, unique_chars):
    model = Sequential()
    model.add(LSTM(128, input_shape=(maxlen, 150), return_sequences=True))
...
def fitModel(maxlen, unique_chars, token_dict_char_int, token_dict_int_char, chars, x, y):
    ...
    model.load_weights('LSTM_new_GPU.h5') # Load existing weights to be trained further
    global model2
    model2 = model
    ...
    model.save("trump_model_fine_tuned.h5")
    return model
...
def main():
    ...
    preprocessed_tweets = []
    f = open('../realDonaldTrump_tweets.csv', encoding="ISO-8859-1")
    for line in f:
        preprocessed_tweets.append(preprocess(line))
    ...
    # model = getModel(maxlen2, unique_chars2)
    # model.load_weights('trump_model_fine_tuned.h5')
    ...
    #             generated = ''
    #             sentence = chars2[start_index: start_index + maxlen2]
    #             generated += sentence
    #             try:
    #                 print('----- Generating with seed: "' + sentence + '"')
    #             except:
