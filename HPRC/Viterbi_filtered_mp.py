...
class Markov:
  ...
  def initialize(self, tweet_lines, tag_lines, threshold):
    ...
    num_removed_words = self.filterWordsCount(threshold) # only keep words with at least n occurences
    print("After filtering to ", str(threshold), " counts: ", self.vocab_size)
  ...
  def buildMarkov(self, tweet_file, tag_file, num_tweets, threshold):
    #Initialize Markov
    ...
    ifs.close()
    self.initialize(tweet_lines, tag_lines, threshold)
...
def main():
  ...
  if len(args) == 3: # sequence length, num tweets, filter threshold    
    length = int(args[0])
    num_tweets = int(args[1])
    threshold = int(args[2])
    words_file = "words_length" + str(length) + ".txt"
    tags_file = "tags_length" + str(length) + ".txt"
    output_file = "filteredOut_length" + str(length) + "_size" + str(num_tweets) + "_filter" + str(threshold) + ".txt"
    
    markov = Markov()
    markov.buildMarkov(words_file, tags_file, num_tweets, threshold)
