...
def main():
  ...
  if len(args) == 3:
    ...
    if args[0] == "load":
      ...
      #tag_sequence_file = "length" + str(length) + "tags.txt"
      #ifs = open(tag_sequence_file, "r")
      #lines = ifs.read().split("\n")
      #ifs.close()
      #tag_sequences = [None for x in range(len(lines))]
      #for i in range(len(lines)):
      ...
      #tag_sequences = [["NN", "RB", "JJ", "PRP$", "NNS"], 
      #                 ["JJ", "NN", "IN", "CD", "NN"], 
      #                 ["IN", ":", "JJ", "NNS", "NN"], 
      #                 ["NN", "RB", "TO", "NN", "NN"], 
      #                 ["NN", "TO", "VB", "DT", "NN"], 
      ...
      #for tag_sequence in tag_sequences:
      #for tag in markov.tags:
      #  tag_sequence = [tag for x in range(length)]
      #  tweet = generate_tweet(tag_sequence, trellis, bp, markov)
      #  #print(tag_sequence)
      #  for i in range(len(tweet)):
      #    print(markov.recoverWord(tweet[i]), sep="", end="")
      #    if i < len(tweet)-1:
      #      if tag_sequence[i+1] != ":" and tag_sequence[i+1] != "." and markov.recoverWord(tweet[i+1]) != "n't" and markov.recoverWord(tweet[i+1]) != "'m":
      #        print(" ", sep="", end="")
      #  print()
